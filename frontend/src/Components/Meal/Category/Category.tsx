...
import { Heading, Flex, Link, Spacer, Stack, VStack, HStack } from '@chakra-ui/react';
...
const MealCategories: React.FC = () => {
  const [selectedCategory, setSelectedCategory] = useState<number>(DEFAULT_CATEGORY);
  const [categoryCount, setCategoryCount] = useState<number>(6);

  //   Using non-useFetch, grabbing placeholder meals from our database
  const [suggestedRecipes, setSuggestedRecipes] = useState<IRecipe[]>([]);
  ...
  return (
    <Fragment>
      <Flex align='end' mt='10' mb='5'>
        ...
        <Spacer />
        <Link as={ReactLink} to='#' _hover={{ color: 'testYellow' }} onClick={handleCategoriesOnClick}>
          {categoryCount > 6 ? 'Show Less' : 'Show All'}
        </Link>
      </Flex>
      <Flex direction='row' flexWrap='wrap' justifyContent='center' gap={6} mb='1rem'>
      ...
      <Flex justifyContent='space-around' direction='row' align='center' flexWrap='wrap' gap={2} pb='2rem'>
