...
const MealPreview: React.FC<MealPreviewProps> = ({ recipe }) => {
  ...
  return (
    <ReactLink to={`/recipes/${id}`}>
      ...
      >
        ...
        <Flex flexDir='column' justifyContent='center' lineHeight='1.2' height='45px'>
          <Text color='white' fontWeight='bold' textAlign='center' noOfLines={2} _groupHover={{ color: 'black' }}>
            {recipe.title}
          </Text>
        </Flex>
        ...
        <Box color='testYellow' lineHeight='1' _groupHover={{ color: 'white' }}>
          ...
          {/* Not as elegant as Array.apply, but I don't know how to fix its TypeScript error! */}
          {renderedStars}
        </Box>
