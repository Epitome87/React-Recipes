...
import { ClockCircleFilled, FireFilled, StarFilled, HeartFilled } from '@ant-design/icons';
import { recipeService } from '../api/recipes.service';
import { getRecipeById } from '../api/recipeSearch';
import useFetch from '../utils/hooks/useFetch';
import axios from 'axios';

const ScreenRecipe: React.FC = () => {
  ...
  const handleFavoriteOnClick = () => {
    toast({
      ...
      description: isFavorite ? 'Meal added to Favorites!' : 'Meal removed from Favorites',
      status: 'success',
      duration: 3000,
      isClosable: true,
  ...
  if (isLoading)
    return (
      <Container maxW='container.xl' minH='93vh' py={10} centerContent bgColor='primary'>
        <Stack direction='row'>
  ...
  return (
    <Container maxW='container.xl' minH='93vh' py={10} centerContent bgColor='primary'>
      ...
      <HStack gap='5rem' mb='3rem'>
        <Stack lineHeight='1' justifyContent='center' alignItems='center' gap='-1rem'>
      ...
      {currentView === 'instructions' &&
        (recipe.analyzedInstructions.length > 0 ? (
          <OrderedList>
            {recipe.analyzedInstructions[0].steps.map((step: { step: string }) => (
      ...
      {currentView === 'ingredients' && (
        <Flex flexWrap='wrap' gap={4}>
          {recipe.extendedIngredients.map((ingredient: { id: string; original: string }) => (
            <Text key={ingredient.id} py={2} px={4} bg='secondary' borderRadius='xl' boxShadow='lg'>
              {ingredient.original}
            </Text>
          ))}
        </Flex>
